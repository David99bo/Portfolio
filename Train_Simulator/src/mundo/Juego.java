...
public class Juego {
	...
	private int wave;
	private ArrayList<Tren> enemigos;
	...
	public boolean trainHitEnemy() {
		boolean booleano =false;
		for(int i= 0; i<enemigos.size();i++) {
			//39x19
			if((tren.getPosX()+39 >= enemigos.get(i).getPosX() && tren.getPosX() <= enemigos.get(i).getPosX()+39)&&(tren.getPosY()+19 >= enemigos.get(i).getPosY() && tren.getPosY() <= enemigos.get(i).getPosY()+19)) {
				booleano = true;
				enemigos.get(i).setDetenido(true);
			}
		}
	...
	public ArrayList<Tren> getEnemigos() {
		return enemigos;
	}



	public void setEnemigos(ArrayList<Tren> enemigos) {
	...
	public void generarEnemigos(){
			ArrayList<Integer> verticesSpawn = new ArrayList<>();
			for(int k = 0; k < rutas.length; k++) {
				if(rutas[Integer.parseInt(tren.getSalida().getCodigo())][k].getCamino().size() != 0 && vertices.get(k).getTipo().equals(Vertex.VERTEX_ESTACION)) {
					verticesSpawn.add(k);
				}
			}
			enemigos = new ArrayList<>();
			ArrayList<Integer> containcheck = new ArrayList<>();
			int i = 0;
			while(containcheck.size()<3) {
				Random ran=new Random();
				int j=ran.nextInt(verticesSpawn.size());
				Vertex temp=vertices.get(verticesSpawn.get(j));
				if(!containcheck.contains(verticesSpawn.get(j))){
				enemigos.add(new Tren(temp.getPosX()+10, temp.getPosY()+10, Tren.DIR_ABAJO, Tren.ENEMIGO));
				enemigos.get(i).setInicio(temp);
				enemigos.get(i).setSalida(temp);
				i++;
				containcheck.add(verticesSpawn.get(j));
				}
			}
			ArrayList<Integer> verticesMeta = new ArrayList<>();
			for(int k = 0; k < rutas.length; k++) {
				boolean contains = containcheck.contains(k);
				if(vertices.get(k).getTipo().equals(Vertex.VERTEX_ESTACION) && !contains) {
					verticesMeta.add(k);
				
				}
			}
			for(int c = 0; c< enemigos.size();c++) {
				Random ran=new Random();
				int j=ran.nextInt(verticesMeta.size());
				Vertex temp=vertices.get(verticesMeta.get(j));
				enemigos.get(c).setMeta(temp);
				int a =Integer.parseInt(enemigos.get(c).getInicio().getCodigo());
				int b = Integer.parseInt(enemigos.get(c).getMeta().getCodigo());
				NodoDijkstra d;
				if(tipo.equals(Juego.TIPO_MATRIZ)){
					d=dijkstra(grafoMatrizAd, a, b);
				}
				else{
					d=dijkstra(corregirMatriz(convertirLista(grafoListaAd)), a, b);
				}
				
				enemigos.get(c).setCamino(d.getCamino());
				enemigos.get(c).setLlegada(vertices.get(d.getCamino().get(1)));
	...
	public void iniciarTren(){
		...
		tren=new Tren(actual.getPosX()+10, actual.getPosY()+10, Tren.DIR_ABAJO, Tren.JUGADOR);
		tren.setSalida(actual);
		tren.setActual(actual);
		tren.setInicio(actual);
		if(tipo.equals(TIPO_MATRIZ)){
			
			rutas=floydWarshall(grafoMatrizAd);
		}
		else{
			
			rutas=floydWarshall(corregirMatriz(convertirLista(grafoListaAd)));
		}
		ArrayList<Integer> verticesLejos = new ArrayList<>();
		for(int k = 0; k < rutas.length; k++) {
			if(rutas[Integer.parseInt(tren.getSalida().getCodigo())][k].getCamino().size() >= 3 && vertices.get(k).getTipo().equals(Vertex.VERTEX_ESTACION)) {
				verticesLejos.add(k);
			}
		}
	...
	public String BFS(int[][] grafoMatrizAd) throws QueueException {
		...
		for(int j = 0; j < vertices.size(); j++) {
			...
			if(grafoMatrizAd[Integer.parseInt(u.getCodigo())][j] != 99999 && vertices.get(j).getColor()==Vertex.BLANCO) {
	...
	public NodoDijkstra[][] getRutas() {
		return rutas;
	}



	public void setRutas(NodoDijkstra[][] rutas) {
		this.rutas = rutas;
	}
