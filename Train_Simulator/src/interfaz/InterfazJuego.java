...
//		JFileChooser fc = new JFileChooser("./data");
	...
	public void iniciarHilosEnemigos(){
		ArrayList<HiloTrenEnemigo> hilirrisEnemy = new ArrayList<>();

		for(int i = 0; i<mundo.getEnemigos().size() ;i++) {
			mundo.getEnemigos().get(i).setDetenido(false);
			hilirrisEnemy.add(new HiloTrenEnemigo(this, mundo.getEnemigos().get(i)));
			hilirrisEnemy.get(i).start();
		}
	...
	public void generarReporte(String reporte) throws IOException{
		
		try {
			String timeStamp = new SimpleDateFormat("yyyyMMdd_HHmmss").format(Calendar.getInstance().getTime());
			BufferedWriter bw = new BufferedWriter(new FileWriter("./data/Reportes/reporte"+timeStamp+".txt"));
			bw.write(reporte);
			bw.close();
		} catch (Exception e) {
			// TODO Auto-generated catch block
