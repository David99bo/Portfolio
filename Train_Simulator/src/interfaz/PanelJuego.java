...
//		g2.drawImage(new ImageIcon("./data/fondo1.png").getImage(),0,0,this);
		...
		for(int i = 0; i<principal.getEnemigos().size(); i++) {
			g2.drawImage(new ImageIcon(principal.getEnemigos().get(i).getImagen()).getImage(), principal.getEnemigos().get(i).getPosX()+3, principal.getEnemigos().get(i).getPosY(), this);
		}
	...
	public void jugarOtraVez(){
		tren.getMeta().setImagen("./data/Escenario/Estacion.png");
		principal.setCamino(new ArrayList<Vertex>());
		cronometro=new Cronometro(frame);
		principal.iniciarTren();
		tren=principal.getTren();
		seleccionado=tren.getActual();
		cambiarSeleccionadoDer();
	}
	@Override
